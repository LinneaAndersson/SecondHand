...
class LoadingSceneController extends Entity {
	...
	LoadingSceneController(final LoadingScene loadingScene,
			final SceneController sceneController, final AllScenes nextScene) {
		super();
		...
		loadingScene.attachChild(new LoadingTextController(loadingScene
				.getLoadingTextView()));

		if (this.nextScene == AllScenes.MAIN_MENU_SCENE) {
			loadingScene.getLoadingTextView().startAnimation(
	...
	private class LoadingDoneCallBack implements LoadingSceneCallback {
		...
		public void doWork() {
			...
			if (nextScene == AllScenes.MAIN_MENU_SCENE) {
				...
				sceneController.getSceneManager()
						.getScene(AllScenes.GAME_PLAY_SCENE).loadScene();
			}
