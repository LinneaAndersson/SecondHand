...
public class Circle extends Shape {
	...
	public Vector2 closest_point_on_seg(final Vector2 seg_a, final Vector2 seg_b) { // NOPMD
		...
		final Vector2 pt_v = new Vector2(this.position.x - seg_a.x,
				this.position.y - seg_a.y);

		final Vector2 seg_v_unit = new Vector2(seg_v.x / seg_v.len(), seg_v.y
				/ seg_v.len());
		...
		if (proj >= seg_v.len())
			return seg_b;
		final Vector2 proj_v = new Vector2(seg_v_unit.x * proj, seg_v_unit.y
				* proj);
	...
	public boolean intersects(final Vector2 seg_a, final Vector2 seg_b) {
		...
		final Vector2 dist_v = new Vector2(this.position.x - closest.x,
