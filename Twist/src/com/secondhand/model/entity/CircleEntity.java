...
public abstract class CircleEntity extends Entity {	
	...
	public Vector2 getSurfacePosition(final Vector2 relativePosition, final boolean farSide) {
		final Vector2 entityCenterPosition = new Vector2(getCenterX(), getCenterY());
		
		// Factor is negative if surfacePosition is far side of circle
		final float factor = (farSide ? -1 : 1 ) * getRadius() / relativePosition.dst(entityCenterPosition);
		
		final float surfaceX = entityCenterPosition.x + factor * (relativePosition.x - entityCenterPosition.x); 
		final float surfaceY = entityCenterPosition.y + factor * (relativePosition.y - entityCenterPosition.y);
