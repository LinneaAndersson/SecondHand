...
public final class PolygonUtil {
	...
	public static boolean isConvex(final List<Vector2> polygon) {
		...
		if (polygon.size() < 3) {
			throw new IllegalArgumentException(
					"A polygon with less than 3 points is not a polygon!");
		}
		...
		for (int i = 0; i < polygon.size(); ++i) {
			...
			/*
			 * from the definition of the cross product(if we set the
			 * z-component of both vectors to 0), we find that the given polygon
			 * is convex only if all the computed z-components(zcrossproduct)
			 * are all positive or all negative. The proof of this is trivial
			 * and left as an excecise to the reader :-)
			 */
			if (!firstPairChecked) {
		...
		// now we've checked all the adjacent pairs of edges, so it must be
		// convex.
		return true;
	}
	...
	public static List<Vector2> getRandomPolygon() {
		...
		for (int i = 0; i < TRANSFORMATIONS; ++i) {
			...
			final Vector2 v = polygonEdges
					.get(rng.nextInt(polygonEdges.size()));

			// randomly tranform it by some random vector.

			Vector2 transformation;
			if (i < TRANSFORMATIONS / 2) {
				transformation = new Vector2((float) rng.nextDouble()
						* MAX_TRANSFORMATION, (float) rng.nextDouble()
						* MAX_TRANSFORMATION);
			} else {
				transformation = new Vector2((float) nextFloat(rng)
						* MAX_TRANSFORMATION, (float) nextFloat(rng)
						* MAX_TRANSFORMATION);
			}
			...
			/*
			 * If this transformation the polygon non-convex, we'll have to undo
			 * it, since Box2D does not support non-convex polygons.
			 */
		...
		// now we have to make sure that the polygon ends up approximately at
