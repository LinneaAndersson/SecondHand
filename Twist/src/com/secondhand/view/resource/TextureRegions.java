...
public class TextureRegions {
	...
	public void load() {
		planetTextures = new EnumMap<PlanetType, TextureRegion>(
				PlanetType.class);

		for (final PlanetType planetType : PlanetType.values()) {
			final TextureRegion planetTexture = TextureRegionLoader
					.getInstance().loadTextureRegion(
							TEXTURE_BASEPATH + planetType.getPath(),
							planetType.getWidth(), planetType.getHeight(),
							TextureOptions.REPEATING_BILINEAR);
			planetTextures.put(planetType, planetTexture);
		}

		powerUpTextures = new EnumMap<PowerUpType, TextureRegion>(
				PowerUpType.class);

		for (final PowerUpType powerUpType : PowerUpType.values()) {
			final TextureRegion planetTexture = TextureRegionLoader
					.getInstance().loadTextureRegion(
							TEXTURE_BASEPATH + powerUpType.getPath(), 64, 64,
							TextureOptions.REPEATING_BILINEAR);
			powerUpTextures.put(powerUpType, planetTexture);
		}

		this.obstacleTexture = TextureRegionLoader.getInstance()
				.loadTextureRegion(TEXTURE_BASEPATH + "obstacle.png", 256, 256,
						TextureOptions.REPEATING_BILINEAR);

		this.starsTexture = TextureRegionLoader.getInstance()
				.loadTextureRegion(TEXTURE_BASEPATH + "stars.png", 256, 256,
						TextureOptions.BILINEAR_PREMULTIPLYALPHA);

		this.playerSprite = TextureRegionLoader.getInstance()
				.loadTextureRegion(TEXTURE_BASEPATH + "player.png", 256, 256,
						TextureOptions.BILINEAR_PREMULTIPLYALPHA);

		this.enemySprite = TextureRegionLoader.getInstance().loadTextureRegion(
				TEXTURE_BASEPATH + "enemy.png", 256, 256,
				TextureOptions.BILINEAR_PREMULTIPLYALPHA);

		this.rocketParticleTexture = TextureRegionLoader.getInstance()
				.loadTextureRegion(TEXTURE_BASEPATH + "particle.png", 256, 256);
